...
const CardContainer = () => {
  ...
  return (
    <div className="h-auto w-full flex flex-col md:p-20 py-32 gap-4 px-4 ">
      ...
      <div
        className="relative md:h-96 md:pl-48 flex gap-8 justify-center  h-52 overflow-x-hidden  md:max-w-screen w-full mt-4 scroll-smooth"
        ref={containerRef}
      >
        {duplicatedToursData.map((tour: any, index: number) => (
          ...
          >
            <div className="rounded-md md:w-72 md:h-72 w-36 h-36  md:px-0  mb-2 product-item">
              ...
              <Image
                ...
                alt={tour.title}
                className="w-full h-full shadow-lg md:object-cover hover:shadow-black/40 rounded-2xl  mb-4 relative "
              />
            </div>
            <span className="uppercase text-center z-50 mx-auto text-xs md:text-sm !text-black">
              {tour.title}
            </span>
          </div>
        ))}
      </div>
      <div className="flex justify-center  md:mt-12 bg-transparent z-10 gap-2 -mb-10">
